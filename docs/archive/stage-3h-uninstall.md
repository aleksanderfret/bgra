> **Archive copy.** English-only plan used to build BGA Stage 3H.
> **Stage:** 3H.
> **Origin:** Cursor plan for Stage 3H (uninstall), after brainstorming + premortem
> critical review (Data vs Application, TEMP NSIS UI, deferred self-delete, gate allowlist).
> **Outcome:** implemented — shared uninstall core, four opt-in checkboxes,
> Mac in-app + Uninstall helper, Windows NSIS → same UI. Mac + Windows smoke
> still required before treating a release as verified.
